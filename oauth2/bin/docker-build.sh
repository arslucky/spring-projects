...
docker rm -f $KAFKA1
docker run -dp $KAFKA_PORT1:$KAFKA_PORT1 --network $NETWORK --network-alias $KAFKA_HOST1 --hostname $KAFKA_HOST1 --name $KAFKA1 \
    -e ALLOW_PLAINTEXT_LISTENER=yes \
    -e KAFKA_ENABLE_KRAFT=no \
    -e KAFKA_CFG_ZOOKEEPER_CONNECT=$ZOO_HOST:$ZOO_PORT \
    -e KAFKA_CFG_LISTENERS=PLAINTEXT://:$KAFKA_PORT1 \
    -e KAFKA_CFG_ADVERTISED_LISTENERS=PLAINTEXT://:$KAFKA_PORT1 \
    bitnami/kafka:3.3.2

docker rm -f $KAFKA2
docker run -dp $KAFKA_PORT2:$KAFKA_PORT2 --network $NETWORK --network-alias $KAFKA_HOST2 --hostname $KAFKA_HOST2 --name $KAFKA2 \
    -e ALLOW_PLAINTEXT_LISTENER=yes \
    -e KAFKA_ENABLE_KRAFT=no \
    -e KAFKA_CFG_ZOOKEEPER_CONNECT=$ZOO_HOST:$ZOO_PORT \
    -e KAFKA_CFG_LISTENERS=PLAINTEXT://:$KAFKA_PORT2 \
    -e KAFKA_CFG_ADVERTISED_LISTENERS=PLAINTEXT://:$KAFKA_PORT2 \
    bitnami/kafka:3.3.2

docker rm -f $KAFKA3
docker run -dp $KAFKA_PORT3:$KAFKA_PORT3 --network $NETWORK --network-alias $KAFKA_HOST3 --hostname $KAFKA_HOST3 --name $KAFKA3 \
    -e ALLOW_PLAINTEXT_LISTENER=yes \
    -e KAFKA_ENABLE_KRAFT=no \
    -e KAFKA_CFG_ZOOKEEPER_CONNECT=$ZOO_HOST:$ZOO_PORT \
    -e KAFKA_CFG_LISTENERS=PLAINTEXT://:$KAFKA_PORT3 \
    -e KAFKA_CFG_ADVERTISED_LISTENERS=PLAINTEXT://:$KAFKA_PORT3 \
    bitnami/kafka:3.3.2
        
echo "creating topic log"

docker exec -it $KAFKA1 /opt/bitnami/kafka/bin/kafka-topics.sh --create --topic $LOG_TOPIC --bootstrap-server $KAFKA_HOST1:$KAFKA_PORT1
